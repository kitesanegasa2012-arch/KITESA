...
st.markdown(
    ...
    <style>
    .main {
        background: linear-gradient(135deg,import streamlit as st
...
def teacher_dashboard_screen():
  ...
  if not students:
    ...
    for name, data in students.items():
      ...
      csv_data += (
          f"{name},{data['grade']},{ao},{math},{eng},{total},{percentage:.1f}%,\""
          f"{summary_text}\"\n"
      )
...
if st.session_state.current_page == "role_selection":
  ...
  teacher_dashboard_screen() #e8f5e9 0%, #c8e6c9 100%);
    }
    .stButton>button {
        background-color: #2E7D32;
        color: white;
...
def teacher_dashboard_screen():
  ...
  if not students:
    ...
    st.text_area("Gabaasa Excel (CSV Format)", csv_data, height=180)
